...
+(NSString *)convertSpeedUnit:(float)kmphspeed{
    ...
    //NSLog(@"convertSpd===%f",convertSpd);
    if ([AppUserDefaults isKMPH]){
        NSString *res = [NSString stringWithFormat:@"%.f%@",convertSpd,NSLocalizedString(@"km_speedUnit", nil)];
        //NSLog(@"res=====%@",res);
        return res;
    }else{
        NSString *res = [NSString stringWithFormat:@"%.f%@",convertSpd,NSLocalizedString(@"mph_speedUnit", nil)];
        //NSLog(@"res=====%@",res);
        return res;
    }
}
