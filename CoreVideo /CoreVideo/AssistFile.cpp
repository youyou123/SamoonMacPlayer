...
int CAssistFile::ParseLine(char *pszLine, int key,int mode)
{
    int				x, y, z;
    char gpsinfo[1024] = {0};
                ...
                do {
                    ...
                    for(i = 0; i < gps_rec_cnt; i++) {
                        ...
                        {	//Decrypt
                            ...
                            //@liangfu 20170228 如果可以多写一次 则先判断第一个字节是否是空格，如果不是就要减去两次key。
                            if((p[0] - key) != ' ')
                                key = 2 * key;
...
int CAssistFile::ParseAssistDataForNovatek66x(string strMOVFile)
{
    char	tFile[PATH_MAX]	 = {0};
    char	szFile[PATH_MAX] = {0};
    FILE	*fd				 = NULL;
            ...
            if(check_flag == 1) {
                ...
                do {
                    ...
                    //@liangfu 20160804.
                    {
                        ...
                        printf("6	Founded %d stsz indexs! mdat_addr:0x%08X, curr:0x%08X\n", size, mdat_addr, ftell(fd));
...
//@20130627
int CAssistFile::ParseMOVSubtitle(int  mode,string strMOVFile)
{
    char	tFile[PATH_MAX]	 = {0};
    char	szFile[PATH_MAX] = {0};
    FILE	*fd				 = NULL;
