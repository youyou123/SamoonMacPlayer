...
NSString *tutvideo=@"http://www.nextbase.co.uk/412GWtutvideo";
...
@property (weak) IBOutlet NSButton *fullscreen;
...
//================
  //  [self.navigateView.layer setBackgroundColor:[[NSColor blackColor] CGColor]];
    //dialog help
...
//    self.helpAbout.hidden=YES;
    ...
    self.currentTimeField.stringValue=@"0:00";
    self.totalTimeField.stringValue=@"0:00";
    [self.captureButton setToolTip:@"Screen Capture to 'Pictures'"];
    [self.fullscreen setToolTip:@"Expand playback window"];
    //modify by chenjian
    //self.fullscreen.hidden = YES;
    //[_volumSlider floatValue];
    NSNumber *vNumValue=[[NSUserDefaults standardUserDefaults] objectForKey:@"volume"];
    if(vNumValue != nil){
        self.volumSlider.floatValue=[vNumValue floatValue];
    }else{
        _volumSlider.floatValue=1.0;
    }
    
    
    //NSLog(@"_volumSlider===%f",_volumSlider.floatValue);
    //dialog help
    //settings
    if([AppUserDefaults isKMPH]){
        
        //NSLog(@"AppUserDefaults isKMPH=====");
        [self.radioSpeed  setState:1 atRow:0 column:0];
        [self.radioSpeed setState:0 atRow:0 column:1];
        
    }else{
        //NSLog(@"AppUserDefaults is not KMPH=====");
    ...
    //defouat image
      NSImage  *norImage=[NSImage imageNamed:@"colorblack"];//defautImage introduce
...
CGEventRef myCallBack(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *userInfo)
{
    ...
    //NSLog(@"myCallBack inputed string:%@, flags:%lld", inputedString, flag);
    return event;
}
...
- (void)setUpPlaybackOfAsset:(AVAsset *)asset withKeys:(NSArray *)keys
{
    ...
    for (NSString *key in keys)
    {
        ...
        {
            NSLog(@"setUpPlaybackOfAsset error %@",error);
            return;
        }
    }
    ...
    if (![asset isPlayable] || [asset hasProtectedContent])
    {
        ...
        NSLog(@"setUpPlaybackOfAsset 无法播放该视频");
        return;
    }
    
      NSLog(@"setUpPlaybackOfAsset -------------------------------------------------------------------------");
    ...
    if ([[asset tracksWithMediaType:AVMediaTypeVideo] count] != 0)
    {
      ...
      //  [self.defaultImage  setHidden:YES];
        NSImage  *norImage=[NSImage imageNamed:@"colorblack"];//defautImage
        [self.defaultImage  setImage:norImage];
        ...
        [self setTimeObserverToken:[[self player] addPeriodicTimeObserverForInterval:CMTimeMake(1, 100) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
            ...
            if(CMTimeGetSeconds(playerItem.asset.duration)==CMTimeGetSeconds(time)){
                ...
                [weakSelf.videoEndDelegate videoEnd];//removesourceMap
                
                   [weakSelf.removesourceMap removeLineMap];
            }
        ...
        NSNumber *vNumValue=[[NSUserDefaults standardUserDefaults] objectForKey:@"volume"];
      //  self.player.volume=self.volumSlider.floatValue;
       //  [self setVolume:vNumValue];
        
        //modify by chenjian
        //if ([vNumValue floatValue]==0.0) {
          //  self.volumSlider.floatValue=1.0;
        //}
        self.volumSlider.floatValue=[vNumValue floatValue];
        //NSLog(@"play self.volumSlider.floatValue----------------%f:", self.volumSlider.floatValue);
        self.player.volume = self.volumSlider.floatValue;
        
//        if(self.player.volume>0){
//            [self.voiceBtn setImage:[NSImage imageNamed:@"speaker_on"]];
//        }else{
//            [self.voiceBtn setImage:[NSImage imageNamed:@"speaker_off"]];//speaker_off
//        }
...
- (IBAction)playOrPause:(id)sender {
    ...
    j=0;
     // [self.player play];
    //NSButton *btn=(NSButton *)sender;
    //NSLog(@"NSButton state: %ld",btn.state);
...
-(NSString *)pictureSaveDirectory{
    NSArray *paths=NSSearchPathForDirectoriesInDomains(NSDocumentationDirectory,NSUserDomainMask,YES);//NSDocumentationDirectory
    NSString *docDir=[paths objectAtIndex:0];
    //NSLog(@"pictureSaveDirectory...%@",docDir);
    NSString *fileName=[NSHomeDirectory() stringByAppendingPathComponent:@"Pictures/"];
    //NSLog(@"pictureSaveDirectory....%@",fileName);
    NSString *pictureDir=[NSString stringWithFormat:@"%@/replay",fileName];
...
- (void)setVolume:(float)volume
{
    ...
    NSLog(@"setVolume-----%f",volume);
      [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithFloat:volume] forKey:@"volume"];
      //modify by chenjian
...
- (IBAction)zoomInOut:(NSButton *)sender {
    NSLog(@"VideoViewController zoomInOut === sender=%@",sender);
...
//    [delegate.windowVC.window toggleFullScreen:@"Document Window Controller"];
    ...
    if(zoomState == 0){
        NSLog(@"更新指南针和速度========");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"compass_notification" object:nil];
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    else if (context == AVSPPlayerRateContext)
    {
        ...
        if(rate==0.f){
            //NSLog(@"ppplayerer=================");
            [_playButton setImage:[NSImage imageNamed:@"player_play"]];//3-1-1//player_pause1-7
            ...
            [_timeSlider setEnabled:YES];
             ...
             //NSLog(@"pppause=================");
            [_playButton setImage:[NSImage imageNamed:@"player_pause"]];//1-7//player_play
            //[_timeSlider setEnabled:NO];
            //modify by chenjian
            [_timeSlider setEnabled:YES];
        }
...
-(void)keyDown:(NSEvent *)theEvent{
    //NSLog(@"video keyDown====%@",theEvent);
    if([theEvent.characters isEqualToString:@" "]){
        [self execPlayOrPause];
    }
    if (theEvent.keyCode==53) {
        //NSLog(@"theEvent.keyCode==53========================");
...
- (IBAction)videoHelpClick:(id)sender {
    ...
    NSURL * url = [NSURL URLWithString:tutvideo];
...
- (IBAction)stopVideo:(id)sender {
      ...
      // [self.videolayer removeFromSuperlayer];
    // [_playButton setImage:[NSImage imageNamed:@"1-7"]];
//    self.currentTimeField.stringValue=@"0:00";
//    self.totalTimeField.stringValue=@"0:00";
   
//_timeSlider.doubleValue =0.0;
    [self.player pause];
    [self setCurrentTime:0.f];
...
//- (IBAction)mapRadio:(NSMatrix *)sender {
//    if(sender.selectedColumn==0){
//        //NSLog(@"mapRadio sender.selectedColumn==0");
//        AppDelegate *delegate=[[NSApplication sharedApplication] delegate];
