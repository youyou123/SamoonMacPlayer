...
- (void)drawRect:(NSRect)dirtyRect {
   ...
   -2 ｜
    ...
    [fattr setObject:[NSFont systemFontOfSize:9] forKey:NSFontAttributeName];
     [fattr setObject:[NSColor whiteColor] forKey:NSForegroundColorAttributeName];//
    
//AppColorManager gsensorBackgroundColor
    NSString *numbers[]={@"-2G",@"-1G",@"0",@"+1G",@"+2G"};
    for(int i=0;i<5;i++){
    ...
    if(_gsensorArray!=nil&&[_gsensorArray count]>=(index+1)){
        ...
        for (int i=0;i<[subArr count];i++) {
            float xValue=[subArr[i][0] floatValue]-1;
